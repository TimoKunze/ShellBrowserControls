...
LRESULT AboutDlg::OnInitDialog(UINT /*message*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*wasHandled*/)
{
	...
	controls.specialThanks_Henk = GetDlgItem(IDC_SPECIALTHANKSTOHENK);
	controls.specialThanks_Jim = GetDlgItem(IDC_SPECIALTHANKSTOJIM);
	controls.specialThanks_Nikos.SubclassWindow(GetDlgItem(IDC_SPECIALTHANKSTONIKOS));
	...
	if(properties.pOwner) {
		...
		controls.specialThanks_Nikos.SetHyperLink(TEXT("http://www.zabkat.com/"));
		controls.specialThanks_Nikos.SetToolTipText(TEXT("Open Nikos' website www.zabkat.com."));
	}
