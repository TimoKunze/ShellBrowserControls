...
HRESULT EnumOLEVERB::CreateInstance(const OLEVERB* pVerbs, int numberOfVerbs, IUnknown* pOuter, REFIID requiredInterface, LPVOID* ppInterfaceImpl)
{
	if(!ppInterfaceImpl) {
		return E_POINTER;
	}
...
HRESULT STDMETHODCALLTYPE EnumOLEVERB::Next(ULONG numberOfMaxVerbs, OLEVERB* pVerbs, ULONG* pNumberOfVerbsReturned)
{
	...
	for(int verb = properties.nextVerb; (verb < properties.numberOfVerbs) && (copiedVerbs < numberOfMaxVerbs); ++verb, ++copiedVerbs) {
		...
		pVerbs[copiedVerbs].lpszVerbName = static_cast<LPWSTR>(CoTaskMemAlloc(bufferSize * sizeof(WCHAR)));
